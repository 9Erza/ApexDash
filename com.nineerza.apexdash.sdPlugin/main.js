...
const udpReceiver = dgram.createSocket({ type: "udp4", reuseAddr: false });
...
// Local debug endpoint. Useful when Stream Dock UI only says NO DATA.
function startDebugServer() {
  ...
  server.on("error", (e) => log("Debug HTTP error: " + e.message));
  server.listen(DEBUG_HTTP_PORT, "127.0.0.1", () => {
...
function connectStreamDock() {
  ...
  websocket.on("open", () => {
    ...
    log("ApexDash v0.4.3 data-only started");
  });
